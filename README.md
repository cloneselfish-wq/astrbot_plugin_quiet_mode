...
- 🗣 **对话式登记机器人**：管理员在群里 `@` 对方说一句「他也是机器人」，插件就把该 QQ 写进防循环名单并**持久化**——不用再手改配置。**v1.7.0 起默认走规则兜底**（不依赖模型是否愿意调工具，实测纯靠 LLM 经常只顾聊天），同时保留 LLM 工具与 `/标记bot`、`/取消标记bot` 指令两条通路
...
| `admin_qqs` | `[]` | 额外管理员 QQ 号；与 **AstrBot 全局管理员**（配置里的「管理员ID」）取并集 |
...
| `bot_guard_auto_register` | `true` | 规则兜底登记：管理员 `@` 某人说「是机器人」时插件**直接登记**，不依赖 AI 调工具 |
...
- **仅 AstrBot 管理员可操作**。口径 = AstrBot 全局管理员（配置里的「管理员ID」，即 `cmd_config.json` 的 `admins_id`）∪ 本插件 `admin_qqs` 白名单，**与 QQ 群主/管理员无关**。
