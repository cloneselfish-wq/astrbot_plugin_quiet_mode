...
- 🗣 **对话式登记机器人**：管理员在群里 `@` 对方说一句「他也是机器人」，LLM 就会调用插件注册的工具把该 QQ 写进防循环名单并**持久化**——不用再手改配置；另提供 `/标记bot`、`/取消标记bot` 指令兜底
...
不想打开 WebUI 改配置时，**群管理员**可以直接在群里登记：

| 你说 | 效果 |
|---|---|
| `@某bot 他也是机器人` | LLM 识别意图 → 调用工具 → 该 QQ 写入 `bot_qq_list` 并持久化 |
| `/标记bot @某bot` | 同上（显式指令，不依赖模型理解） |
| `/取消标记bot @某bot` | 从名单移除 |
| `/标记bot` | 列出当前名单 |
...
- **仅管理员可操作**。工具内部严格校验身份（`admin_qqs` 命中或 AstrBot 内置管理员），普通群友即使诱导 LLM 也会被拒绝——防止真人被误标成机器人（后果包括 bot 不再引用其消息、甚至被连续 bot 消息熔断波及）
