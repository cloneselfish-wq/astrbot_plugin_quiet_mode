...
- **权限分级：全局指令仅 AstrBot 管理员，单群指令对所有人开放。**
  - **全局指令**（对话式的 `全群闭嘴` / `全群张嘴` / `全群恢复`，以及 `/quiet_global_silent`、`/quiet_global_resume`、`/quiet_clear_groups`）**恒定仅 AstrBot 全局管理员**可触发。全群闭嘴一次影响所有会话，属高影响动作，因此不接受插件侧白名单，也**不受 `admin_only` 影响**——即使把 `admin_only` 关掉也照样拦住。
  - **单群指令**（人格关键词 + 触发词，如 `沙绫闭嘴`）**默认任何人**都能触发；只有把 `admin_only` 打开（`true`）才会收紧为仅管理员。
  - 无权限触发仍是**静默丢弃**（不回话，避免把规则暴露给普通群友），但日志由 `debug` 提升为 `info` 并带上 `scope=`，方便"喊了没反应"时定位。

### Fixed

- `admin_only` 的默认值由 `true` 改为 `false`，与上述"单群人人可用"的产品定位一致（**已存在的配置文件不受默认值影响，需手动改**）。
- 全局指令被拒文案由「仅管理员可操作」改为「仅 AstrBot 管理员可操作全局设置」，明确口径。

## [1.7.2] - 2026-09-10

### Fixed

- **修 v1.7.0 引入的致命 bug：规则兜底登记一执行就抛 `name 're' is not defined`**。`_keyword_hit` 里用了 `re.search` / `re.escape`，但 `main.py` 顶部漏了 `import re`；异常被方法自己的 `try/except` 吞掉，外部表现是「在群里 @ 对方说了是 BOT，名单却一直没变，日志里只有一行 ERRO」。同一 bug 也让 `/标记bot` 的正则兜底取号失效。
- **普通聊天不再误触发全局闭嘴/张嘴**。默认全局恢复词里的「恢复」过于宽泛——别人一句「好的，我恢复说话了~」就会命中，插件把 `global_quiet` 切成 False 并**发出一段莫名其妙的感言**（实测日志：`切换全局闭嘴 → False` + `对话式感言生成成功 action=resume`）。现在三道防线：
  - 全局触发词必须「近乎整句」出现才生效：去掉触发词、@ 段、标点后，剩余内容 ≤ `global_trigger_max_residual`（默认 4）字。「全群张嘴」剩余为空 → 正常触发；「好的，我恢复说话了~」剩余「好的我说话了」 → 不触发。
  - 默认全局恢复词由 `["全群张嘴", "全局张嘴", "恢复"]` 改为 `["全群张嘴", "全局张嘴", "全群恢复", "全局恢复"]`，移除泛词「恢复」。
  - **其他 bot 的发言不再被当成闭嘴/张嘴指令**（发送者命中 `bot_qq_list` 时跳过指令解析），避免 bot 之间互相切状态。

### Changed

- 闭嘴/张嘴**状态没有实际变化时不再生成感言**（重复指令、误命中一律静默）。
- 切换日志由 `admin=` 改为 `sender=`，并追加「状态已变更 / 状态未变」——`admin_only=false` 时触发者未必是管理员，旧措辞会误导排查。
